...
bool HasPrefix(const CString& name, LPCTSTR prefix)
{
...
bool RenumberResourceHeader(LPCTSTR szSrcPath, LPCTSTR szDstPath, const CResourceFile& resourceFile)
{
    ...
    while (fIn.ReadString(line)) {
        ...
        if (iPos >= 0) {
            ...
            int	iVal;
            if (resourceFile.idMap.GetCount() && !resourceFile.idMap.Lookup(sName, iVal)) {
    ...
    if (!AfxWinInit(::GetModuleHandle(NULL), NULL, ::GetCommandLine(), 0)) {
        ...
        if (argc > 1) {
            TRY{
                ...
                RenumberResourceHeader(argv[1], CString(argv[1]) + ".new", resourceFile);
            }
                CATCH(CException, e) {
